...
async def get_services():
    try:
        services = await cfg.dbconnector_is.callproc('is_services_get', rows=-1, values=[None, None])
        for s in services:
            service_processes = await cfg.dbconnector_is.callproc(s['serviceName']+'_processes_get', rows=-1, calues=[None, None, None, None, None])
...
async def get_service(service_name):
    try:
        service = await cfg.dbconnector_is.callproc('is_services_get', rows=1, values=[service_name, None])
        processes = await cfg.dbconnector_is.callproc(f"{service_name}_processes_get", rows=-1, values=[service_name, None, None, None, None])
