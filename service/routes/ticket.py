    ...
    if re.match("[0-9]{11}", ticket) or re.match("[0-9]{24}"):
        try:
            if operation == 'reactivate':
                if len(ticket) == 11:
                    tasks.add_task(cfg.dbconnector_wp.callproc, 'rem_ticket_reactivate', rows=0, values=[None, ticket])
                    return Response(status_code=200, media_type='application/json', background=tasks)
                elif len(ticket) == 24:
                    tasks.add_task(cfg.dbconnector_wp.callproc, 'rem_ticket_reactivate', rows=0, values=[ticket, None])
                    return Response(status_code=200, media_type='application/json', background=tasks)
            elif operation == 'free':
                if len(ticket) == 11:
                    tasks.add_task(cfg.dbconnector_wp.callproc, 'rem_ticket_free', rows=0, values=[None, ticket])
                    return Response(status_code=200, media_type='application/json', background=tasks)
                elif len(ticket) == 24:
                    tasks.add_task(cfg.dbconnector_wp.callproc, 'rem_ticket_free', rows=0, values=[ticket, None])
                    return Response(status_code=200, media_type='application/json', background=tasks)
            else:
