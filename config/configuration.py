...
config_file = (str(Path(str(Path(__file__).parents[2]) + "/config.ini")))
...
### AMPP DEVICE MAPPING ###
device_mapping = str(Path(str(Path(__file__).parents[1]) + "devices.json"))

### LOG FILES ###

if not os.path.isdir(str(Path(str(Path(__file__).parents[0]) + "/logs"))):
    os.mkdir((str(Path(str(Path(__file__).parents[0]) + "/logs"))))

sys_log = str(Path(str(Path(__file__).parent) + "/logs/sys.log"))
...
amqp_password = parser.get("AQMP", "password")
