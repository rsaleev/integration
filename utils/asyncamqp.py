...
from aio_pika.exceptions import ChannelClosed, ChannelNotFoundEntity, DeliveryError, PublishError, AMQPConnectionError, AuthenticationError, AMQPError, AMQPConnectionError, ChannelInvalidStateError
import json
import asyncio
...
class AsyncAMQP:
    ...
    async def connect(self):
        while self.__cnx is None or self.__ch is None:
            try:
                self.__cnx = await connect(f"amqp://{self.__user}:{self.__password}@{self.__host}/", loop=asyncio.get_running_loop())
    ...
    async def receive(self, processor):
        try:
            async with self.__q.iterator() as q:
                async for message in q:
                    async with message.process():
                        ...
                        await processor(redelivered, key, data)
        except (ConnectionError, ConnectionRefusedError, ConnectionResetError, ConnectionAbortedError, TimeoutError, RuntimeError):
            await self.connect()
